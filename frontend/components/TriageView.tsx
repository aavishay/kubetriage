...
export const TriageView: React.FC<TriageViewProps> = ({ workloads, isDarkMode = true, onOpenChat, defaultTemplate: propTemplate, initialWorkloadId: propId }) => {
  ...
  const triggerAutoAnalysis = async (workload: Workload, playbook: DiagnosticPlaybook) => {
    ...
    try {
      const { analysis } = await analyzeWorkload(workload, playbook, aiConfig.provider, aiConfig.model);
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-zinc-50/20 dark:bg-black/20 pb-32">
              ...
              <div className="flex flex-col gap-10">
                    ...
                    {isAnalyzing ? <div className="h-full flex flex-col items-center justify-center text-center gap-6"><Loader2 className="w-12 h-12 text-indigo-500 animate-spin" /><h4 className="text-lg font-black text-zinc-900 dark:text-white uppercase tracking-tighter">AI SRE Analysis...</h4></div> : analysis ? <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
                      ...
                      <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-500/30 backdrop-blur-xl rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 relative z-10">
                          ...
                          <div>
                            <h4 className="text-lg font-black text-zinc-900 dark:text-white uppercase tracking-tighter">Analysis Complete</h4>
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 font-bold">Report has been generated and archived.</p>
                          </div>
                        </div>
                        <Link to="/reports" className="group flex items-center gap-2 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-sm hover:shadow-md transition-all border border-zinc-200 dark:border-zinc-700">
                          View in Reporting Center <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                        </Link>
                      </div>

                      <div className="mt-8 prose prose-sm prose-indigo max-w-none dark:prose-invert prose-headings:font-black prose-headings:tracking-tighter prose-p:leading-loose prose-strong:text-indigo-600 dark:prose-strong:text-indigo-400 prose-li:marker:text-zinc-400">
                ...
                <section className="bg-zinc-950 rounded-[3rem] border border-zinc-800 shadow-2xl overflow-hidden flex flex-col min-h-[500px]">
                  <div className="px-6 py-5 border-b border-zinc-800/50 bg-black/40 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                  ...
                  <div className="p-6 overflow-auto font-mono text-[11px] leading-relaxed flex-1 bg-black">
