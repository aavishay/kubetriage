...
const SEVERITY_CONFIG: Record<string, { bar: string; badge: string; dot: string }> = {
  Critical: { bar: 'bg-danger shadow-[0_0_8px_#e74c3c]', badge: 'kt-badge-danger', dot: 'bg-danger shadow-[0_0_4px_#e74c3c]' },
  High: { bar: 'bg-warning shadow-[0_0_8px_#f5a623]', badge: 'kt-badge-warning', dot: 'bg-warning shadow-[0_0_4px_#f5a623]' },
  Medium: { bar: 'bg-warning/70', badge: 'kt-badge-warning', dot: 'bg-warning/70' },
  Low: { bar: 'bg-info', badge: 'kt-badge-info', dot: 'bg-info' },
};
const DEFAULT_SEVERITY = { bar: 'bg-text-tertiary', badge: 'bg-text-tertiary/10 text-text-tertiary border-border-main', dot: 'bg-text-tertiary' };
const PAGE_SIZE = 50;
...
const markdownComponents = {
  ...
  code: ({ inline, className, children, ...props }: any) => {
    if (inline) {
      return (
        <code className="bg-primary-500/10 text-primary-500 px-1 py-0.5 rounded-sm text-[11px] font-mono" {...props}>
          {children}
        </code>
      );
    }
    return (
      <div className="my-2 overflow-hidden border border-border-main kt-panel-inset">
...
const IncidentRow = memo(function IncidentRow({ incident }: IncidentRowProps) {
  ...
  useEffect(() => {
    ...
    if (el) setIsDescriptionClamped(el.scrollHeight > el.clientHeight + 1);
  }, [cleanedDescription]);

  useEffect(() => {
    const el = rootCauseRef.current;
    if (el) setIsRootCauseClamped(el.scrollHeight > el.clientHeight + 1);
  }, [cleanedRootCause]);
  ...
  const copyToClipboard = useCallback(async () => {
    ...
    } catch (err) { console.error('Failed to copy incident description', err); }
        ...
        {incident.rootCause && (
          <div className="mt-2.5 flex items-start gap-1.5 text-[11px] text-text-tertiary font-sans">
            ...
            {isExpanded ? (
              ...
              <span ref={rootCauseRef} className="line-clamp-1">{cleanedRootCause}</span>
            )}
          </div>
        )}
      </div>
...
export const MultiClusterIncidentsList: React.FC<MultiClusterIncidentsListProps> = memo(function MultiClusterIncidentsList({ incidents }) {
  ...
  React.useEffect(() => { setVisibleCount(PAGE_SIZE); }, [debouncedSearch, severityFilter, patternFilter]);
  ...
  const clearFilters = useCallback(() => { setSearchTerm(''); setSeverityFilter('all'); setPatternFilter('all'); }, []);
  ...
  const handleSeverityChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => { setSeverityFilter(e.target.value); }, []);
  const handlePatternChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => { setPatternFilter(e.target.value); }, []);
      ...
      <div className="kt-panel-header">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input type="text" placeholder="Search incidents..." value={searchTerm} onChange={handleSearchChange} className="kt-input pl-9 pr-7 py-1.5 text-xs w-full sm:w-56" />
            {searchTerm && (
              <button onClick={clearSearch} className="absolute right-2 top-1/2 -translate-y-1/2 text-text-tertiary hover:text-text-primary">
