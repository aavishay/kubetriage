...
// GitOpsArgoCDDiffHandler returns a compact diff for all ArgoCD Applications.
func GitOpsArgoCDDiffHandler(c *gin.Context) {
	clusterID := c.Query("cluster")

	cacheKey := fmt.Sprintf("gitops:argocd:diff:%s", clusterID)
	if val, err := cache.Get(c.Request.Context(), cacheKey); err == nil {
		c.Header("X-Cache", "HIT")
		c.Data(http.StatusOK, "application/json", []byte(val))
		return
	...
	apps, _ := gitops.DiffArgoCDApplications(c.Request.Context(), client)
	response := gitops.ArgoCDDiffResponse{
		ClusterID:    clusterID,
		Timestamp:    time.Now(),
		Applications: apps,
	}
