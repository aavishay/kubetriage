...
export const TriageView: React.FC<TriageViewProps> = ({ workloads, isDarkMode = true, onOpenChat, defaultTemplate, initialWorkloadId }) => {
  ...
  useEffect(() => { if (defaultTemplate) setSelectedPlaybook(defaultTemplate as DiagnosticPlaybook); }, [defaultTemplate]);

  useEffect(() => {
    if (initialWorkloadId) {
      const workload = workloads.find(w => w.id === initialWorkloadId || w.name === initialWorkloadId);
      if (workload) { setSelectedWorkload(workload); setIsSidebarOpen(false); triggerAutoAnalysis(workload, (defaultTemplate as DiagnosticPlaybook) || 'General Health'); }
    }
  }, [initialWorkloadId, workloads, defaultTemplate]);
