...
import { Bell, Plus, Search, MoreHorizontal, Slack, Mail, Webhook, Trash2, Edit2, X, Activity, Loader2, Play, Settings2, ShieldAlert, Cpu, MemoryStick, Zap, DollarSign, Filter, CheckCircle2, AlertCircle, MessageSquare, History, Clock, ArrowRight, BellRing } from 'lucide-react';
...
export const NotificationsView: React.FC<NotificationsViewProps> = ({ 
  ...
  const filteredHistory = triggeredAlerts.filter(a =>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {activeTab === 'channels' ? (
                ...
                filteredRules.map(rule => {
                  ...
                  return (
                    <div key={rule.id} className={`bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 border shadow-sm group hover:border-indigo-500/50 transition-all flex flex-col h-full relative overflow-hidden ${
                      isActive ? 'border-red-500 dark:border-red-900/50' : 'border-zinc-200 dark:border-zinc-800'
                    }`}>
